...
import { FetchAllPostsByCategory } from '../stores/PostSlice'

export default function PostByCategory(props) {
    ...
    useEffect(() => {
        dispatch(FetchAllPostsByCategory(params.slug)).then((data) => {
    ...
    const getMoreData = (page) => {
        if (lastPage > page) {
            ...
            dispatch(FetchAllPostsByCategory(params.slug, page)).then((data) => {
    ...
    return (
        <FrontendLayout>
            <CategoryHeaderTitle category={ allPosts.length ? allPosts[0] : null } />
            <BannerAd />
                            ...
                            <div className="blog-list clearfix">
                                <div className="section-title">
                                    ...
                                    {allPosts.length > 0 && allPosts.map((value, index) => {
                                        return <><HomePostListItem post={value} /><hr className="invis" /></> 
                                    })}
                            </div>
                        </div>
